...
class SvgPathParser {
    ...
    private float xMax = -10000f, xMin = 10000f, yMax = -10000f, yMin = 10000f;
     ...
     */
    float[] copyOfRange(float[] original, int start, int end) {
        if (start > end) {
     ...
     */
    public Path createPathFromPathData(String pathData) {
        Path path = new Path();
        PathDataNode[] nodes = createNodesFromPathData(pathData);
        if (nodes != null) {
            try {
                new PathDataNode().nodesToPath(nodes, path);
     ...
     */
    public PathDataNode[] createNodesFromPathData(String pathData) {
        if (pathData == null) {
            return null;
        }
        int start = 0;
     ...
     */
    public PathDataNode[] deepCopyNodes(PathDataNode[] source) {
        if (source == null) {
            return null;
        }
     ...
     */
    public boolean canMorph(PathDataNode[] nodesFrom, PathDataNode[] nodesTo) {
     ...
     */
    public void updateNodes(PathDataNode[] target, PathDataNode[] source) {
        for (int i = 0; i < source.length; i++) {
    ...
    private void addNode(ArrayList<PathDataNode> list, char cmd, float[] val) {
    ...
    public class PathDataNode {
         ...
         */
        public void nodesToPath(PathDataNode[] node, Path path) {
            ...
            for (int i = 0; i < node.length; i++) {
                ...
                for (int j = 0; j < node[i].mParams.length; j += 2) {
                    try {
                        if (xMax < node[i].mParams[j]) xMax = node[i].mParams[j];
                        if (xMin > node[i].mParams[j]) xMin = node[i].mParams[j];
                        if (yMax < node[i].mParams[j + 1]) yMax = node[i].mParams[j + 1];
                        if (yMin > node[i].mParams[j + 1]) yMin = node[i].mParams[j + 1];
                    } catch (IndexOutOfBoundsException ex) {
                        Log.e(LOGTAG, "nodesToPath: " + ex.getMessage());
                    }
        ...
        private void addCommand(Path path, float[] current, char previousCmd, char cmd, float[] val) {
        ...
        private void drawArc(Path p, float x0, float y0, float x1, float y1, float a, float b, float theta, boolean isMoreThanHalf, boolean isPositiveArc) {
         ...
         */
        private void arcToBezier(Path p, double cx, double cy, double a, double b, double e1x, double e1y, double theta, double start, double sweep) {
